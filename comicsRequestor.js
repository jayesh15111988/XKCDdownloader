...
console.log("remote url "+ remoteComicsFetcherURL + " and remote parameters "+getRequestParametersToSendToServer());
    ...
    sendRequestToServerWithParameters({
        ...
        parameters: getRequestParametersToSendToServer(),
        requestType: 0
    });
...
$("#removeSelectedImagesButton").click(function() {
    ...
    if (confirm('Are you sure you want to remove input range of images from server directory ' + defaultServerFolderName + '?')) {
        sendRequestToServerWithParameters({
            loadingText: "Removing Selected files... Please be patient",
            remoteURL: remoteSelectedDeleteURL,
            parameters: getRequestParametersToSendToServer(),
            requestType: 1
        });
    }
...
$("#removeAllImagesButton").click(function() {
    ...
    if (confirm('Are you sure you want to remove all Comics from server directory ' + defaultServerFolderName + '?')) {
        sendRequestToServerWithParameters({
            loadingText: "Removing all files. Please be patient",
            remoteURL: remoteAllDeleteURL,
            parameters: {
                defaultFolderNameValue: defaultServerFolderName
            },
            requestType: 2
        })
    }
...
function getRequestParametersToSendToServer() {
    return {
        miniComicsSequence: minimumComicsNumber,
        maxComicsSequence: maximumComicsNumber,
        defaultFolderNameValue: defaultServerFolderName
    };
}
